...
public:
	Demo():
		DemoBase(TITLE, SCR_W, SCR_H, SCALE, 0.5, NULL),

		transparentReplaceData{16, 13, 3},
	...
	~Demo() {
		PIF_imageFree(img);
		PIF_imageFree(invertMap);
		PIF_fontFree(font);
	}

private:
	PIF_Image *invertMap, *img;
	PIF_Rect   imgDest;
	PIF_Font  *font;

	TransparentReplaceData transparentReplaceData;

	uint8_t borderColor, brushColor;

	int  brushSize, brushX, brushY, brushPrevX, brushPrevY;
	bool drawing, colorPicker, helpText;
	...
	void draw(int xStart, int yStart, int xEnd, int yEnd) {
		...
		for (float t = 0; t <= 1; t += step) {
			int x = round(lerp(xStart, xEnd, t));
			int y = round(lerp(yStart, yEnd, t));
